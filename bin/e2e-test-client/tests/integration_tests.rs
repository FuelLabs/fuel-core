...
fn dev_config() -> Config {
    ...
    let chain_config = ChainConfig::load_from_directory(
        "../../deployment/scripts/chainspec/dev",
    ).expect("Should be able to load chain config");
    // TODO: don't use serde here
    let state_config = fs::read_to_string(
        "../../deployment/scripts/chainspec/dev/state_config.json",
    ).expect("Should be able to read state config");
    let state_config: StateConfig = serde_json::from_str(&state_config).expect("Should be able to decode state config");
