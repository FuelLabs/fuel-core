...
mod tests {
    ...
    // Coins:
    // * skips groups according to genesis progress
    // * each batch is done in a transaction
    // * stops everything when batch is invalid
    //
    // * updates the db with the coin (whatever is done in init_coin)
    //
    // * coin root table is updated
    // * output idx is incremented for each coin
    //
    // * Genesis progress is incremented after each batch
    // * transaction is committed when the batch is successful
    // * can interrupt the process, upon interruption, the current batch is allowed to finish
    //
    // Messages:
    // * skips groups according to genesis progress
    // * each batch is done in a transaction
    // * stops everything when batch is invalid
    //
    // * updates the db with the message (whatever is done in init_da_message)
    // * message root table is updated
    //
    // * Genesis progress is incremented after each batch
    // * transaction is committed when the batch is successful
    // * can interrupt the process, upon interruption, the current batch is allowed to finish
    ...
    // * stops everything when batch is invalid
    //
    // * updates the db with the contract (whatever is done in init_contract)
    // * output idx is incremented for each contract
    // * contract id table is updated
    //
    // * Genesis progress is incremented after each batch
    // * transaction is committed when the batch is successful
    // * can interrupt the process, upon interruption, the current batch is allowed to finish
    ...
    // * stops everything when batch is invalid
    //
    // * updates the db with the contract state (whatever is done in db.update_contracT_states)
    // * Genesis progress is incremented after each batch
    // * transaction is committed when the batch is successful
    // * can interrupt the process, upon interruption, the current batch is allowed to finish
    //
    ...
    // * stops everything when batch is invalid
    //
    // * updates the db with the contract balance (whatever is done in db.update_contract_balances)
    //
    // * Genesis progress is incremented after each batch
    // * transaction is committed when the batch is successful
    // * can interrupt the process, upon interruption, the current batch is allowed to finish

    trait TransactionOpener {
    ...
    struct GenesisRunner<Handler, Groups, TxOpener> {
    ...
    where
        Handler: FnMut(Group<GroupItem>, &mut Database) -> anyhow::Result<()>,
        ...
        TxOpener: TransactionOpener,
    {
        fn new(
            ...
            let skip = tx_opener.view_only().genesis_progress(&genesis_progress);
            let groups = groups.skip(skip);
            Self {
                handler,
                groups,
                resource: genesis_progress,
                tx_opener,
            }
        }

        fn run(&mut self) -> anyhow::Result<bool> {
            let has_more = match self.groups.next() {
                Some(group) => {
    ...
    #[test]
    fn run_signals_when_no_more_groups() {
        // given
        let groups = std::iter::empty();
        let mut bh = GenesisRunner::new(
            GenesisResource::Coins,
            |_: Group<()>, _| Ok(()),
            groups,
    ...
    #[test]
    fn run_signals_if_groups_remaining() {
        // given
        let groups = (0..2).map(|i| {
            Ok(Group {
                index: i,
                data: vec![],
            })
    ...
    fn seeing_an_invalid_group_propagates_the_error() {
        // given
        let groups = vec![Err(anyhow!("Some error"))];
        let mut bh = GenesisRunner::new(
            GenesisResource::Coins,
            |_: Group<()>, _| Ok(()),
            groups.into_iter(),
