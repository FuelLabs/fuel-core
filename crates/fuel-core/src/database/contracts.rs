...
// # Dev-note: The value of the `ContractsRawCode` has a unique implementation of
// serialization and deserialization. Because the value is a contract byte code
// represented by bytes, we don't use `serde::Deserialization` and `serde::Serialization`
// for `Vec`, because we don't need to store the size of the contract. We store/load raw
// bytes.
            ...
            .map(|raw_contract_id| -> StorageResult<ContractConfig> {
                ...
                let state = Some(
                    ...
                    .map(|res| -> DatabaseResult<(Bytes32, Bytes32)> {
                        ...
                        // We don't need to store ContractId which is the first 32 bytes
                        // of this key, as this Vec is already
                        // attached to that ContractId
