...
mod tests {
    use fuel_core_executor::{
        executor::{ExecutionData, ExecutionOptions, Executor, OnceTransactionsSource},
        refs::ContractRef,
        Config,
    };

    use fuel_core_storage::{
        tables::{Coins, ContractsRawCode, Messages, Receipts, Transactions},
        StorageAsMut,
    };
    ...
    use fuel_core_types::{
        ...
        fuel_tx::{
            field::{
                ...
                Script as ScriptField, TxPointer as TxPointerTraitTrait,
            },
            input::Input,
        ...
        fuel_vm::{
            checked_transaction::CheckError, script_with_data_offset,
            util::test_helpers::TestBuilder as TxBuilder, Call, CallFrame, Contract,
        },
        services::{
            block_producer::Components,
            executor::{
                Error as ExecutorError, ExecutionBlock, ExecutionResult, ExecutionType,
                ExecutionTypes, TransactionExecutionResult, TransactionValidityError,
            },
        },
        tai64::Tai64,
    };

    use crate::{database::Database, query::SimpleTransactionData};
                        ...
                        *block.transactions_mut() = vec![mint.into()];
                        ...
                        assert!(matches!(
                            ...
                            ExecutorError::InvalidTransaction(
                                CheckError::Validity(ValidityError::TransactionMintIncorrectBlockHeight)
                            )
                        ));
                    }
                    ...
                    fn invalidate_invalid_base_asset() {
                        ...
                        *block.transactions_mut() = vec![mint.into()];
                        ...
                        assert!(matches!(
                            validation_err,
                            ExecutorError::InvalidTransaction(
                                CheckError::Validity(ValidityError::TransactionMintNonBaseAsset)
                            )
                        ));
                    }
    ...
    /*
        ...
        fn executor_invalidates_missing_gas_input() {
            ...
            assert!(matches!(
                ...
                &ExecutorError::InvalidTransaction(
                    CheckError::Validity(
                        CheckError::InsufficientFeeAmount { expected, .. }
                    )
                ) if expected == max_fee
            ));
        ...
        fn input_coins_are_marked_as_spent_with_utxo_validation_enabled() {
            ...
            if let Input::CoinSigned(Input::CoinSigned {
                utxo_id,
                owner,
                amount,
                asset_id,
