//! The module defines the `BTreeMerkle` blueprint for the storage.
//! The `BTreeMerkle` blueprint implements the BTree merkle tree on top of the storage.
//! It is like a [`Plain`](super::plain::Plain) blueprint that builds the BTree
...
use crate::{
    ...
    tables::merkle::BTreeMerkleMetadata,
...
pub const BTREE_NODES: u8 = 32;

/// The trait that allows to convert the key of the table into a prefix and unique key.
/// All entries of the BTree Merkle tree are prefixed with the same key, and for
...
/// The `BTreeMerkle` blueprint builds the storage as a [`Plain`](super::plain::Plain)
/// blueprint and maintains the BTree merkle tree by the `Metadata` and `Nodes` tables.
///
...
/// and the `Nodes` table stores the tree's nodes. The BTree Merkle is built over the encoded
/// keys and values using the same encoding as for main key-value pairs.
pub struct BTreeMerkle<Metadata, Nodes, ValueEncoder> {
    _marker: core::marker::PhantomData<(Metadata, Nodes, ValueEncoder)>,
}

impl<Metadata, Nodes, Encoder> BTreeMerkle<Metadata, Nodes, Encoder>
where
    Metadata: Mappable<Value = BTreeMerkleMetadata, OwnedValue = BTreeMerkleMetadata>,
    ...
    Nodes::Key: PrefixedKey,
{
    ...
    where
        ...
        S: StorageMutate<Metadata, Error = StorageError>
            + BTreeStorage<Nodes, StorageError = StorageError>,
    {
        let (prefix, unique) = key.prefix();
        let metadata_key = prefix.into();
        // Get latest metadata entry for this `primary_key`
        let prev_metadata: Cow<BTreeMerkleMetadata> = storage
            .storage::<Metadata>()
            .get(&metadata_key)?
            .unwrap_or_default();

        let root_node = prev_metadata.into_owned().root_node();
        let mut tree: BTreeMerkleTree<BTREE_NODES, _, Nodes> =
            BTreeMerkleTree::load(prefix, root_node, storage);

        let encoded_value = Encoder::encode(value);
        tree.insert(unique, encoded_value.as_bytes())
            .map_err(|err| StorageError::Other(anyhow::anyhow!("{err:?}")))?;

        // Generate new metadata for the updated tree
        let (root_node, storage) = tree.into_inner();
        let metadata = BTreeMerkleMetadata::new(root_node);
        storage
            .storage::<Metadata>()
            ...
            + BTreeStorage<Nodes, StorageError = StorageError>,
    {
        ...
        let metadata_key = prefix.into();
        // Get latest metadata entry for this `primary_key`
        let prev_metadata: Option<Cow<BTreeMerkleMetadata>> =
...
impl<M, S, Metadata, Nodes, Encoder> BlueprintInspect<M, S>
    for BTreeMerkle<Metadata, Nodes, Encoder>
where
    M: Mappable,
    ...
    Self: BlueprintInspect<M, S>,
{
    fn root(storage: &S, key: &Metadata::Key) -> StorageResult<MerkleRoot> {
        let metadata: Option<Cow<BTreeMerkleMetadata>> =
            storage.storage_as_ref::<Metadata>().get(key)?;
        let root = metadata
            .and_then(|metadata| metadata.into_owned().root_node())
            .map(|mut root| root.hash())
            .unwrap_or_else(|| [0; 32]);
        Ok(root)
    }
}

impl<S, M, StorageError> BTreeStorage<M> for StorageInterlayer<S>
where
