...
async fn insert__tx_with_blacklisted_utxo_id() {
    ...
    // Given
    let (_, coin) = universe.setup_coin();
...
async fn insert_higher_priced_tx_removes_lower_priced_tx() {
    ...
    // Given
    let (_, coin_input) = universe.setup_coin();
    let tx1 = universe.build_script_transaction(Some(vec![coin_input.clone()]), None, 10);
    let tx_id = tx1.id(&ChainId::default());
    let tx2 = universe.build_script_transaction(Some(vec![coin_input]), None, 20);
