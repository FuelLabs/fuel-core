        ...
        + TxIdOwnerRecorder<Error = StorageError>,
{
    ...
    fn execute_mint(
        ...
        if mint.input_contract().contract_id == ContractId::zeroed() {
            ...
            let mut vm_db =
                sub_db_view.new(&header.consensus, self.config.coinbase_recipient);
        ...
        <Tx as IntoChecked>::Metadata: Fee + CheckedMetadata + Clone + Send + Sync,
    {
        ...
        let vm_db = sub_db_view.new(&header.consensus, self.config.coinbase_recipient);
...
mod tests {
    ...
    #[cfg(test)]
    impl Executor<MockStorage, MockStorage> {
        fn test(database: MockStorage, config: Config) -> Self {
            Self {
    ...
    impl RelayerPort for MockStorage {
        fn get_message(&self, _id: &Nonce, _da_height: &DaBlockHeight) -> anyhow::Result<Option<Message>> {
            todo!()
        }
    }
    ...
    use fuel_core_database::test_helpers::MockStorage;
    ...
    fn executor_validates_correctly_produced_block() {
        ...
        dbg!(validation_result.is_ok());
        dbg!(&skipped_transactions.is_empty());
