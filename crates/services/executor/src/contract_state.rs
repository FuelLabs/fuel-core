...
pub fn compute_balances_hash(
    ...
    let mut touched_assets: BTreeMap<ContractId, BTreeMap<AssetId, Word>> =
        BTreeMap::new();
    for r in record {
        if r.column == Column::ContractsAssets as u32 {
            ...
            touched_assets.entry(*contract_id).or_default().insert(
                *asset_id,
                r.value
                    .clone()
                    .map(|v| {
...
pub fn compute_state_hash(
    ...
    let mut touched_slots: BTreeMap<ContractId, BTreeMap<Bytes32, Option<Vec<u8>>>> =
        BTreeMap::new();
    for r in record {
        if r.column == Column::ContractsState as u32 {
            ...
            touched_slots
                .entry(*contract_id)
                .or_default()
                .insert(*state_key, r.value.clone());
        }
    }
