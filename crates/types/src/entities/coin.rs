...
/// The `Coin` is either `CoinStatus::Spent` or `CoinStatus::Unspent`. If the coin is
/// unspent, it can be used as an input to the transaction and can be spent up to the
/// `amount`. After usage as an input of a transaction, the `Coin` becomes
/// `CoinStatus::Spent`.
