...
- [2260](https://github.com/FuelLabs/fuel-core/pull/2260): Upon starting the node, start migrating all key-value pairs in the table `modification_history` to `modification_history_v2`. Migration changes are accumulated by a dedicated thread in a in-memory transaction, and they are committed to the historical RocksDB only when changes for a new height need to be committed, or when changes must be reverted because of a rollback. If all key-value pairs have been migrated, the migration thread is not spawned at node startup.
