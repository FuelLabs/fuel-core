...
#[tokio::test(flavor = "multi_thread")]
async fn try_quorum_50_percent_syncing() {
    let mut config = Config::local_node();
    config.relayer = Some(relayer::Config::default());
    let relayer_config = config.relayer.as_mut().expect("Expected relayer config");
    let eth_node = MockMiddleware::default();
...
#[tokio::test(flavor = "multi_thread")]
async fn quorum_33_percent_fail_syncing() {
    let mut config = Config::local_node();
    config.relayer = Some(relayer::Config::default());
    let relayer_config = config.relayer.as_mut().expect("Expected relayer config");
    let eth_node = MockMiddleware::default();
    ...
    relayer_config.relayer = Some(vec![
        format!("http://{}", eth_node_handle.address)
            ...
            .unwrap(),
        "http://localhost:8080".try_into().unwrap(),
        "http://localhost:8081".try_into().unwrap(),
    ]);
    ...
    let srv = FuelService::from_database(db.clone(), config)
        .await
        .unwrap();
    let max_timeout = std::time::Duration::from_secs(10);

    // When
    tokio::select! {
        _ = srv.await_relayer_synced() => {
            panic!("Should not have synced");
        }
        _ = tokio::time::sleep(max_timeout) => {
            // Then
            // We should not have synced
        }
    }
