...
async fn can_get_message_proof() {
    for n in [1, 2, 10] {
        ...
        for nonce in nonces.clone() {
            ...
            let message_proof_set: Vec<_> =
                result.message_proof.proof_set.iter().cloned().collect();
            ...
            let block_proof_set: Vec<_> =
                result.block_proof.proof_set.iter().cloned().collect();
