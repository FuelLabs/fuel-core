...
pub enum DiscoveryEvent {
    /// Notify the swarm of a Kademlia event
...
impl NetworkBehaviour for DiscoveryBehaviour {
    ...
    fn poll(
        ...
        while let Poll::Ready(kad_action) = self.kademlia.poll(cx, params) {
            match kad_action {
                NetworkBehaviourAction::GenerateEvent(kad_event) => {
                    return Poll::Ready(NetworkBehaviourAction::GenerateEvent(
                        DiscoveryEvent::KademliaEvent(kad_event),
                    ))
                }
        ...
        while let Poll::Ready(mdns_event) = self.mdns.poll(cx, params) {
            match mdns_event {
                NetworkBehaviourAction::GenerateEvent(mdns_event) => match mdns_event {
...
mod tests {
    ...
    use libp2p::{
        core, identity::Keypair, kad::KademliaEvent, multiaddr::Protocol, noise, swarm::SwarmEvent,
                ...
                (0..discovery_swarms.len())
                    ...
                    .filter_map(|swarm_index| {
                        ...
                        if swarm_index != current_index {
                            // get the PeerId
                            Some(Swarm::local_peer_id(&discovery_swarms[swarm_index].0).clone())
                        } else {
                            None
                        }
                    })
        ...
        let test_future = poll_fn(move |cx| {
            'polling: loop {
                for swarm_index in 0..discovery_swarms.len() {
                    match discovery_swarms[swarm_index].0.poll_next_unpin(cx) {
                        Poll::Ready(Some(event)) => {
                            match event {
                                SwarmEvent::Behaviour(discovery_event) => match discovery_event {
