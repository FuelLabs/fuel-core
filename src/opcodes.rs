...
pub enum Opcode {
    ...
    Mlog        (RegisterId, RegisterId, RegisterId)                 = 0x1a,
    MRoo        (RegisterId, RegisterId, RegisterId)                 = 0x1b,
    ...
    CTMV        (RegisterId, RegisterId)                             = 0x31,
    JI          (Immediate24)                                        = 0x32,
    ...
    Aloc        (RegisterId)                                         = 0x44,
    MCl         (RegisterId, RegisterId)                             = 0x45,
    MClI        (RegisterId, Immediate18)                            = 0x46,
    MCp         (RegisterId, RegisterId, RegisterId)                 = 0x47,
...
impl Opcode {
    ...
    pub const fn registers(&self) -> [Option<RegisterId>; 4] {
        match self {
            ...
            Self::MemEq(ra, rb, rc, rd) => [Some(*ra), Some(*rb), Some(*rc), Some(*rd)],
            Self::SB(ra, rb, _) => [Some(*ra), Some(*rb), None, None],
            Self::SW(ra, rb, _) => [Some(*ra), Some(*rb), None, None],
            Self::Blockhash(ra, rb) => [Some(*ra), Some(*rb), None, None],
            Self::Blockheight(ra) => [Some(*ra), None, None, None],
            Self::Burn(ra) => [Some(*ra), None, None, None],
            Self::Call(ra, rb, rc, rd) => [Some(*ra), Some(*rb), Some(*rc), Some(*rd)],
            Self::CodeCopy(ra, rb, rc, rd) => [Some(*ra), Some(*rb), Some(*rc), Some(*rd)],
            Self::CodeRoot(ra, rb) => [Some(*ra), Some(*rb), None, None],
            Self::CodeSize(ra, rb) => [Some(*ra), Some(*rb), None, None],
            Self::Coinbase(ra) => [Some(*ra), None, None, None],
            Self::Loadcode(ra, rb, rc) => [Some(*ra), Some(*rb), Some(*rc), None],
            Self::Log(ra, rb, rc, rd) => [Some(*ra), Some(*rb), Some(*rc), Some(*rd)],
            Self::Mint(ra) => [Some(*ra), None, None, None],
            Self::Revert(ra) => [Some(*ra), None, None, None],
            Self::SLoadCode(ra, rb, rc) => [Some(*ra), Some(*rb), Some(*rc), None],
            ...
            Self::Transfer(ra, rb, rc) => [Some(*ra), Some(*rb), Some(*rc), None],
            Self::TransferOut(ra, rb, rc, rd) => [Some(*ra), Some(*rb), Some(*rc), Some(*rd)],
            Self::ECRecover(ra, rb, rc) => [Some(*ra), Some(*rb), Some(*rc), None],
            Self::Keccak256(ra, rb, rc) => [Some(*ra), Some(*rb), Some(*rc), None],
            Self::Sha256(ra, rb, rc) => [Some(*ra), Some(*rb), Some(*rc), None],
...
impl From<Opcode> for u32 {
    fn from(opcode: Opcode) -> u32 {
        match opcode {
            ...
            Opcode::Mlog(ra, rb, rc) => (0x1a << 24) | ((ra as u32) << 18) | ((rb as u32) << 12) | ((rc as u32) << 6),
            Opcode::MRoo(ra, rb, rc) => (0x1b << 24) | ((ra as u32) << 18) | ((rb as u32) << 12) | ((rc as u32) << 6),
            ...
            Opcode::Aloc(ra) => (0x44 << 24) | ((ra as u32) << 18),
            Opcode::MCl(ra, rb) => (0x45 << 24) | ((ra as u32) << 18) | ((rb as u32) << 12),
            Opcode::MClI(ra, imm18) => (0x46 << 24) | ((ra as u32) << 18) | (imm18 as u32),
            Opcode::MCp(ra, rb, rc) => (0x47 << 24) | ((ra as u32) << 18) | ((rb as u32) << 12) | ((rc as u32) << 6),
